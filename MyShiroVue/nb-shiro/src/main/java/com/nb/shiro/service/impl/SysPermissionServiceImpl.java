...
public class SysPermissionServiceImpl extends ServiceImpl<SysPermissionMapper, SysPermission> implements SysPermissionService {
    ...
    public List<SysPermission> getPermissionTreeList() {
        //查询所有
        List<SysPermission> allList = list(new QueryWrapper<SysPermission>().eq("del_flag",0));
        //根节点
        List<SysPermission> rootList =  allList.stream().filter(item->StringUtils.isEmpty(item.getParentId())).collect(Collectors.toList());
        for(SysPermission root:rootList){
            buildChildrenList(allList,root);
        }
        return rootList;
    }

    private void buildChildrenList(List<SysPermission> allList, SysPermission parentNode) {

        List<SysPermission> childrenList = new ArrayList<>();
        for (SysPermission node : allList) {
            if (StringUtils.equals(node.getParentId(),parentNode.getId())) {
                if(node.getIsLeaf() == 0){
                    buildChildrenList(allList,node);
                }
                //是这个node的子节点
                childrenList.add(node);
                parentNode.setChildren(childrenList);
